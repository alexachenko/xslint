...
src/index.mjs (CLI, commander.js — ESM entry so commander loads natively)
  → src/xslint.js (file discovery, suppression, run order, output: defects to
    ...
    → src/import-linter.js (xsl:import/xsl:include cycles; builds the dependency
        graph via src/import-graph.js) → src/resources/checks/format/circular-import.yaml
...
| `src/import-graph.js` | Resolves `xsl:import`/`xsl:include` hrefs against each file's directory and builds the dependency graph over the corpus; an href outside the corpus is external, not an edge |
| `src/import-linter.js` | Walks the import graph for `xsl:import`/`xsl:include` cycles (`circular-import`, an error) |
