...
/**
 * The `count(...)`-versus-zero comparisons in an expression: each carries the
 * offset it starts at, its verbatim text, and the existence test that replaces
 * it. A call whose parentheses do not balance, or that is compared with
 * anything but `0`/`1` in an existence-testing way, is skipped.
 * @param {string} expression - The attribute value
...
const comparisons = function(expression) {
  ...
  for (const match of blanked.matchAll(CALL)) {
    ...
    const test = collapses(tail[1], tail[2])
    if (!test) {
      continue
    }
    found.push({
      ...
      replacement: `${test}(${expression.slice(open + 1, close)})`,
    })
  }
  return found
}
